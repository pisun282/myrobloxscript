-- Полная очистка всех старых потоков и элементов интерфейса для предотвращения наложений
...
-- Базовые настройки конфигурации и состояний
...
local flyKey = Enum.KeyCode.F -- КЛАВИША ПОЛЕТА ПО УМОЛЧАНИЮ [F]
local blockFly = false
local guiVisible = true
local toggleGuiKey = Enum.KeyCode.Insert
local lastFlyState = false

-- Глобальные переменные для удержания физических объектов оригинального полета
local bv, bg
...
circleBtn.Size = UDim2.new(0, 55, 0, 45)
...
stroke.Thickness = 3
stroke.Color = Color3.fromRGB(75, 255, 75)
...
local function lbl(t, s, p, f)
    ...
    l.Size = s l.Position = p l.BackgroundTransparency = 1
    l.Font = f or Enum.Font.SourceSans
    l.TextSize = f and 16 or 14
...
local function box(t, p)
    ...
    b.Size = UDim2.new(0, 60, 0, 30) b.Position = p
    b.BackgroundColor3 = Color3.fromRGB(45, 45, 45)
    b.BorderSizePixel = 0 b.Font = Enum.Font.SourceSansBold
    b.TextSize = 14 b.TextColor3 = Color3.fromRGB(255, 255, 255)
    b.Text = t
    return b
end
...
local function tgl(p)
    ...
    t.Size = UDim2.new(0, 30, 0, 30) t.Position = p
    t.BackgroundColor3 = Color3.fromRGB(255, 75, 75)
    t.Font = Enum.Font.SourceSansBold t.TextSize = 16
...
timeLabelTop.TextColor3 = Color3.fromRGB(255, 255, 255)
...
btn.Size = UDim2.new(0, 180, 0, 35) btn.Position = UDim2.new(0, 15, 0, 60)
btn.BackgroundColor3 = Color3.fromRGB(255, 75, 75) btn.Font = Enum.Font.SourceSansBold
btn.TextSize = 15 btn.TextColor3 = Color3.fromRGB(255, 255, 255)
btn.Text = "ВКЛЮЧИТЬ ПОЛЕТ"

local bBtn = Instance.new("TextButton", mf)
bBtn.Size = UDim2.new(0, 180, 0, 25) bBtn.Position = UDim2.new(0, 15, 0, 105)
bBtn.BackgroundColor3 = Color3.fromRGB(45, 45, 45) bBtn.Font = Enum.Font.SourceSansBold
bBtn.TextSize = 13 bBtn.TextColor3 = Color3.fromRGB(220, 220, 220)
bBtn.Text = "Бинд Fly: [ F ]"
...
local sInp = box(tostring(wSpd), UDim2.new(0, 315, 0, 60))
local jInp = box(tostring(jPwr), UDim2.new(0, 315, 0, 105))
local sTgl = tgl(UDim2.new(0, 385, 0, 60))
local jTgl = tgl(UDim2.new(0, 385, 0, 105))

local afkBtn = Instance.new("TextButton", mf)
afkBtn.Size = UDim2.new(0, 420, 0, 35) afkBtn.Position = UDim2.new(0, 15, 0, 185)
afkBtn.BackgroundColor3 = Color3.fromRGB(255, 75, 75) afkBtn.Font = Enum.Font.SourceSansBold
afkBtn.TextSize = 14 afkBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
afkBtn.Text = "ВКЛЮЧИТЬ АНТИ-АФК"

local timeLabel = Instance.new("TextLabel", mf)
timeLabel.Size = UDim2.new(0, 200, 0, 30) timeLabel.Position = UDim2.new(0, 15, 0, 225)
timeLabel.BackgroundTransparency = 1 timeLabel.Font = Enum.Font.SourceSansBold
timeLabel.TextSize = 15 timeLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
timeLabel.Text = "Время АФК: 00:00:00" timeLabel.TextXAlignment = Enum.TextXAlignment.Left

local rBtn = Instance.new("TextButton", mf)
rBtn.Size = UDim2.new(0, 150, 0, 30) rBtn.Position = UDim2.new(0, 285, 0, 225)
rBtn.BackgroundColor3 = Color3.fromRGB(55, 55, 55) rBtn.Font = Enum.Font.SourceSansBold
rBtn.TextSize = 13 rBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
rBtn.Text = "СБРОС" rBtn.BorderSizePixel = 0

local espBtn = Instance.new("TextButton", mf)
espBtn.Size = UDim2.new(0, 420, 0, 30) espBtn.Position = UDim2.new(0, 15, 0, 265)
espBtn.BackgroundColor3 = Color3.fromRGB(255, 75, 75) espBtn.Font = Enum.Font.SourceSansBold
espBtn.TextSize = 13 espBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
espBtn.Text = "ESP УЛЬТРА: ВЫКЛ" espBtn.BorderSizePixel = 0

local godBtn = Instance.new("TextButton", mf)
godBtn.Size = UDim2.new(0, 420, 0, 30) godBtn.Position = UDim2.new(0, 15, 0, 305)
godBtn.BackgroundColor3 = Color3.fromRGB(255, 75, 75) godBtn.Font = Enum.Font.SourceSansBold
godBtn.TextSize = 13 godBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
godBtn.Text = "РЕЖИМ БОГА (GODMODE): ВЫКЛ" godBtn.BorderSizePixel = 0
local function toggleGui()
    guiVisible = not guiVisible
    mf.Visible = guiVisible
    stroke.Color = guiVisible and Color3.fromRGB(75, 255, 75) or Color3.fromRGB(255, 75, 75)
end

-- ОРИГИНАЛЬНЫЙ СТАБИЛЬНЫЙ МЕТОД ПОЛЕТА ИЗ ТВОЕГО СКРИПТА
local function toggleFly() 
    ...
    fly = not fly 
    if fly then 
        btn.Text = "ВЫКЛЮЧИТЬ ПОЛЕТ" 
        btn.BackgroundColor3 = Color3.fromRGB(75, 255, 75) 
        h.PlatformStand = true 
        
        -- Создание оригинальной стабилизации
        ...
        -- ЖЁСТКИЙ ФИКС НАКЛОНА КУКЛЫ ПРИ ОТКЛЮЧЕНИИ ОРИГИНАЛЬНОГО ПОЛЁТА
...
-- АБСОЛЮТНЫЙ РЕЖИМ БОГА (Силовое поле движка)
local function toggleGodMode()
    godModeToggle = not godModeToggle
    if godModeToggle then
        godBtn.BackgroundColor3 = Color3.fromRGB(75, 255, 75)
        godBtn.Text = "РЕЖИМ БОГА (GODMODE): ВКЛ"
        _G.XenoGodModeLoop = R.Heartbeat:Connect(function()
            if godModeToggle and lp.Character then
        ...
        godBtn.BackgroundColor3 = Color3.fromRGB(255, 75, 75)
        godBtn.Text = "РЕЖИМ БОГА (GODMODE): ВЫКЛ"
        if _G.XenoGodModeLoop then _G.XenoGodModeLoop:Disconnect() _G.XenoGodModeLoop = nil end
        if lp.Character and lp.Character:FindFirstChildOfClass("ForceField") then
            lp.Character:FindFirstChildOfClass("ForceField"):Destroy()
        end
    end
end
...
local function updateESP()
    ...
    for _, p in ipairs(P:GetPlayers()) do
        if p ~= lp and p.Character and p.Character:FindFirstChildOfClass("Humanoid") and p.Character:FindFirstChildOfClass("Humanoid").Health > 0 and p.Character:FindFirstChild("HumanoidRootPart") then
            ...
            hl.Name = "XenoSkinESP" hl.Parent = p.Character
            ...
            local distance = 0
            if myRoot then distance = math.floor((myRoot.Position - targetRoot.Position).Magnitude) end
            local bb = Instance.new("BillboardGui", p.Character)
            bb.Name = "XenoTextESP" bb.AlwaysOnTop = true bb.Size = UDim2.new(5, 0, 2, 0)
            bb.ExtentsOffset = Vector3.new(0, 3, 0) bb.Adornee = targetRoot
            local tl = Instance.new("TextLabel", bb)
            tl.Size = UDim2.new(1, 0, 1, 0) tl.BackgroundTransparency = 1
            tl.Font = Enum.Font.SourceSansBold tl.TextSize = 13 tl.TextColor3 = Color3.fromRGB(255, 255, 255)
...
espBtn.MouseButton1Click:Connect(function()
    ...
    espBtn.Text = espToggle and "ESP УЛЬТРА: ВКЛ" or "ESP УЛЬТРА: ВЫКЛ"
    espBtn.BackgroundColor3 = espToggle and Color3.fromRGB(75, 255, 75) or Color3.fromRGB(255, 75, 75)
    updateESP()
end)
...
afkBtn.MouseButton1Click:Connect(function() afkToggle = not afkToggle afkBtn.BackgroundColor3 = afkToggle and Color3.fromRGB(75, 255, 75) or Color3.fromRGB(255, 75, 75) afkBtn.Text = afkToggle and "АНТИ-АФК: РАБОТАЕТ" or "ВКЛЮЧИТЬ АНТИ-АФК" updT() end)

-- Кнопка СБРОСА времени с подтверждением при работающем Анти-АФК
rBtn.MouseButton1Click:Connect(function() 
    if not afkToggle then 
        afkSeconds = 0 updT() 
    else 
        if not resetConfirm then 
            resetConfirm = true confirmTime = tick() 
            rBtn.Size = UDim2.new(0, 250, 0, 30) rBtn.Position = UDim2.new(0, 185, 0, 225) 
            rBtn.BackgroundColor3 = Color3.fromRGB(200, 150, 50) rBtn.TextSize = 11 
            rBtn.Text = "Вы точно хотите сбросить время?" 
        else 
            resetConfirm = false afkSeconds = 0 updT() 
            rBtn.Size = UDim2.new(0, 150, 0, 30) rBtn.Position = UDim2.new(0, 285, 0, 225) 
            rBtn.BackgroundColor3 = Color3.fromRGB(55, 55, 55) rBtn.TextSize = 13 
            rBtn.Text = "СБРОС" 
        end 
    end 
end)

-- Фоновые асинхронные задачи и слушатели циклов игры
...
local ev = R.RenderStepped:Connect(function() 
    ...
    if h and r and not fly then h.UseJumpPower = true end 
    if flyKey and not isBind and not blockFly and not U:GetFocusedTextBox() then 
        local isDown = U:IsKeyDown(flyKey) 
        if isDown and not lastFlyState then toggleFly() end 
        lastFlyState = isDown 
    end 
    if U:IsKeyDown(Enum.KeyCode.Space) and jumpToggle and not fly and h and r then 
        if h:GetState() ~= Enum.HumanoidStateType.Jumping and h:GetState() ~= Enum.HumanoidStateType.Freefall then 
            r.AssemblyLinearVelocity = Vector3.new(r.AssemblyLinearVelocity.X, jPwr, r.AssemblyLinearVelocity.Z) 
        end 
    end 
end)

-- ОРИГИНАЛЬНЫЙ ИСПРАВЛЕННЫЙ ЦИКЛ НОУКЛИПА ПРИ ФЛАЕ
task.spawn(function()
    while true do
        task.wait()
...
-- Кнопка полного стирания скрипта (Фикс багов)
...
hBtn.MouseButton1Click:Connect(function() if ev then ev:Disconnect() end if ev2 then ev2:Disconnect() end if _G.XenoToggleGuiConn then _G.XenoToggleGuiConn:Disconnect() end fly, speedToggle, jumpToggle, afkToggle, espToggle = false, false, false, false, false clearESP() if bv then bv:Destroy() end if bg then bg:Destroy() end local h = lp.Character and lp.Character:FindFirstChildOfClass("Humanoid") if h then h.PlatformStand = false h.WalkSpeed = 16 h.JumpPower = 50 end sg:Destroy() end)
